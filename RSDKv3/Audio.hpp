...
extern int masterVolume;      // engine music-fade level (VAR_MUSICVOLUME); transient, not persisted
extern int userMasterVolume;  // user Master Volume setting (Options); scales music + sfx
extern int trackID;
...
// --- audio-reactive UI meters (written on the audio thread, read by the UI) --
// Smoothed envelopes in the 0..1 range derived from the final mixed output, so
// the remastered menu (and anything else) can pulse in time with the music.
extern float audioMeterPeak; // broadband loudness envelope
extern float audioMeterBass; // low-frequency (beat) envelope
