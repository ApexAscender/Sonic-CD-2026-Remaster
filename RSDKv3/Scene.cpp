...
void LoadStageFiles(void)
{
    ...
    if (!CheckCurrentStageFolder(stageListPosition)) {
        ...
        memset(objectScriptPaths, 0, sizeof(objectScriptPaths));
#endif
        ...
        if (loadGlobalScripts && LoadFile("Data/Game/GameConfig.bin", &info)) {
...
#if RETRO_USE_NATIVE_OBJECTS
            // read the global objects' script paths here (they follow the
            // names) so native binding works in bytecode mode too
            for (byte i = 0; i < globalObjectCount; ++i) {
                FileRead(&fileBuffer2, 1);
                FileRead(objectScriptPaths[i + scriptID], fileBuffer2);
                objectScriptPaths[i + scriptID][fileBuffer2] = 0;
            }
#endif
            ...
            if (Engine.usingBytecode) {
                ...
                for (byte i = 0; i < globalObjectCount; ++i) {
#if RETRO_USE_NATIVE_OBJECTS
                    // paths were pre-read above for native binding
                    StrCopy(strBuffer, objectScriptPaths[scriptID]);
#else
                    FileRead(&fileBuffer2, 1);
                    FileRead(strBuffer, fileBuffer2);
                    strBuffer[fileBuffer2] = 0;
...
#if RETRO_USE_NATIVE_OBJECTS
                StrCopy(objectScriptPaths[scriptID], modScriptPaths[i]);
#endif
                GetFileInfo(&infoStore);
                CloseFile();
...
#if RETRO_USE_NATIVE_OBJECTS
            // read the stage objects' script paths here (they follow the
            // names) so native binding works in bytecode mode too
            for (byte i = 0; i < stageObjectCount; ++i) {
                FileRead(&fileBuffer2, 1);
                FileRead(objectScriptPaths[scriptID + i], fileBuffer2);
                objectScriptPaths[scriptID + i][fileBuffer2] = 0;
            }
#endif
...
#if RETRO_USE_NATIVE_OBJECTS
                    // paths were pre-read above for native binding
                    StrCopy(strBuffer, objectScriptPaths[scriptID + i]);
#else
                    FileRead(&fileBuffer2, 1);
                    FileRead(strBuffer, fileBuffer2);
                    strBuffer[fileBuffer2] = 0;
    ...
    Init3DFloorBuffer(0);
#if RETRO_USE_NATIVE_OBJECTS
    // Bind registered native objects to this stage's live type IDs before setup runs.
    InitNativeObjects();
#endif
