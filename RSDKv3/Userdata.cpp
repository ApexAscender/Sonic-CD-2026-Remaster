...
//#elif RETRO_PLATFORM == RETRO_LINUX
  //  sprintf(buffer, "%s/settings.ini", getXDGDataPath().c_str());
    ...
    if (!file) {
        ...
        float bv = 0, sv = 0, mv = 0;
        if (!ini.GetFloat("Audio", "BGMVolume", &bv))
            bv = 1.0f;
        if (!ini.GetFloat("Audio", "SFXVolume", &sv))
            sv = 1.0f;
        if (!ini.GetFloat("Audio", "MasterVolume", &mv))
            mv = 1.0f;

        bgmVolume        = bv * MAX_VOLUME;
        sfxVolume        = sv * MAX_VOLUME;
        userMasterVolume = mv * MAX_VOLUME;               // real user Master setting (NOT engine masterVolume)
        if (userMasterVolume > MAX_VOLUME) userMasterVolume = MAX_VOLUME;
        if (userMasterVolume < 0)          userMasterVolume = 0;
...
void WriteSettings()
{
    ...
    ini.SetFloat("Audio", "MasterVolume", userMasterVolume / (float)MAX_VOLUME);
    ini.SetFloat("Audio", "BGMVolume", bgmVolume / (float)MAX_VOLUME);
    ini.SetFloat("Audio", "SFXVolume", sfxVolume / (float)MAX_VOLUME);
