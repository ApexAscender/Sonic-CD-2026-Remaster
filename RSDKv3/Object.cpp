...
// Run an object sub if it's implemented natively OR has bytecode. Native objects
// must run even with no scripts/bytecode loaded (RetroScript fully removed).
#if RETRO_USE_NATIVE_OBJECTS
#define RUN_SUB(type, sub, codePtr) (NativeObjectHasSub(type, sub) || scriptCode[codePtr] > 0)
#else
#define RUN_SUB(type, sub, codePtr) (scriptCode[codePtr] > 0)
#endif
...
void ProcessStartupObjects()
{
    ...
    for (int i = 0; i < OBJECT_COUNT; ++i) {
        ...
        entity->type                = i;
        if (RUN_SUB(i, SUB_SETUP, scriptInfo->subStartup.scriptCodePtr))
...
void ProcessObjects()
{
    ...
    for (objectLoop = 0; objectLoop < ENTITY_COUNT; ++objectLoop) {
        ...
        if (active && entity->type > OBJ_TYPE_BLANKOBJECT) {
            ...
            activePlayer             = 0;
            if (RUN_SUB(entity->type, SUB_MAIN, scriptInfo->subMain.scriptCodePtr))
                ProcessScript(scriptInfo->subMain.scriptCodePtr, scriptInfo->subMain.jumpTablePtr, SUB_MAIN);
            if (RUN_SUB(entity->type, SUB_PLAYERINTERACTION, scriptInfo->subPlayerInteraction.scriptCodePtr)) {
