...
int masterVolume  = MAX_VOLUME; // ENGINE music-fade level (VAR_MUSICVOLUME): scripts drive
                                // this down for transient fades (time warp, death, signposts,
                                // bosses) and it resets to MAX on every music play. NOT a user
                                // setting — do not persist it or apply it to SFX.
int userMasterVolume = MAX_VOLUME; // real user Master Volume (Options): scales music AND sfx
int trackID       = -1;
...
// audio-reactive UI meters (see Audio.hpp) — updated from ProcessAudioPlayback
float audioMeterPeak = 0.0f;
float audioMeterBass = 0.0f;
...
void ProcessMusicStream(Sint32 *stream, size_t bytes_wanted)
{
    ...
    switch (musicStatus) {
        ...
        case MUSIC_PLAYING: {
            ...
            if (bytes_done != 0)
                ProcessAudioMixing(stream, streamInfoPtr->buffer, bytes_done / sizeof(Sint16),
                                   ((bgmVolume * masterVolume) / MAX_VOLUME) * userMasterVolume / MAX_VOLUME, 0);
#endif
            ...
            if (bytes_gotten > 0) {
                ...
                if (cvtResult == 0)
                    ProcessAudioMixing(stream, (const Sint16 *)convert.buf, bytes_gotten / sizeof(Sint16),
                                       ((bgmVolume * masterVolume) / MAX_VOLUME) * userMasterVolume / MAX_VOLUME, 0);
...
void ProcessAudioPlayback(void *userdata, Uint8 *stream, int len)
{
    ...
    while (samples_remaining != 0) {
...
#if RETRO_USING_SDL2
        // Process music being played by a video (ogv theora, or mp4 via libav)
        if (videoPlaying == 1) {
            // Fetch decoded audio packets and shove them into the SDL Audio Stream.
            // Both decoders output float32 stereo @ 48000 Hz (what ogv_stream expects).
            const size_t bytes_to_do = samples_to_do * sizeof(Sint16);

#if RETRO_USE_MP4
            if (videoIsMp4 && mp4Decoder) {
                Mp4AudioPacket *mpkt;
                while ((mpkt = Mp4_GetAudio(mp4Decoder)) != NULL) {
                    SDL_AudioStreamPut(ogv_stream, mpkt->samples, mpkt->frames * sizeof(float) * 2);
                    Mp4_FreeAudio(mpkt);
                }
            }
            else
...
#if RETRO_USING_SDL1 || RETRO_USING_SDL2
                ProcessAudioMixing(mix_buffer, buffer, (int)samples_done, (sfxVolume * userMasterVolume) / MAX_VOLUME, sfx->pan);
#endif
            }
        }

        // audio-reactive UI meter state (fast attack / slow release envelopes)
        static float s_bassLP  = 0.0f; // one-pole low-pass state (~140 Hz)
        static float s_peakEnv = 0.0f; // broadband envelope
        static float s_bassEnv = 0.0f; // low-band (beat) envelope
        ...
        for (size_t i = 0; i < sizeof(mix_buffer) / sizeof(*mix_buffer); ++i) {
            ...
            // ---- feed the UI meters -------------------------------------------
            float m = (float)sample * (1.0f / 32768.0f);
            if (m > 4.0f) m = 4.0f;
            else if (m < -4.0f) m = -4.0f;
            float am = m < 0 ? -m : m;
            // broadband: instant attack, ~75 ms release
            if (am > s_peakEnv) s_peakEnv += (am - s_peakEnv) * 0.25f;
            else                s_peakEnv += (am - s_peakEnv) * 0.00030f;
            // low band: one-pole LP then its own envelope (~110 ms release)
            s_bassLP += (m - s_bassLP) * 0.010f;
            float ab = (s_bassLP < 0 ? -s_bassLP : s_bassLP) * 2.4f;
            if (ab > s_bassEnv) s_bassEnv += (ab - s_bassEnv) * 0.30f;
            else                s_bassEnv += (ab - s_bassEnv) * 0.00020f;
        }
        audioMeterPeak = s_peakEnv > 1.0f ? 1.0f : s_peakEnv;
        audioMeterBass = s_bassEnv > 1.0f ? 1.0f : s_bassEnv;
