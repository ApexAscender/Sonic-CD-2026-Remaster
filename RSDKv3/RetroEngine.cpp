...
#include "Native/SettingsOverlay.hpp"
#include "Native/RemasterMenu.hpp"
#endif
...
bool ProcessEvents()
{
    ...
    while (SDL_PollEvent(&Engine.sdlEvents)) {
        ...
        switch (Engine.sdlEvents.type) {
            ...
            case SDL_KEYDOWN:
#if RETRO_USE_NATIVE_OBJECTS
                // The settings overlay (F8) gets first refusal on keydowns; when
                // it consumes one, skip the engine's own handling for that key.
                if (SettingsOverlay::HandleKeyDown(Engine.sdlEvents.key.keysym.sym, Engine.sdlEvents.key.keysym.mod))
                    break;
                // remastered Options "Controls" rebind: capture the next key press.
                if (RemasterMenu::OptionsCaptureKey(Engine.sdlEvents.key.keysym.scancode))
                    break;
#endif
                switch (Engine.sdlEvents.key.keysym.sym) {
                    ...
                    // SonicCD-Native: Alt+Enter is the standard fullscreen shortcut
                    // (in addition to F4). Plain Enter is untouched (handled by input).
                    case SDLK_RETURN:
                        if (Engine.sdlEvents.key.keysym.mod & KMOD_ALT) {
                            Engine.isFullScreen ^= 1;
                            SetFullScreen(Engine.isFullScreen);
                        }
                        break;
...
#if RETRO_USE_NATIVE_OBJECTS
                // Remastered boot: play the custom intro video (videos/intro.ogv)
                // before the title, replacing the sprite Sega/Sonic Team screens.
                // It's skippable (Start/A/B/C), and no-ops safely if absent.
                {
                    char introName[] = "intro";
                    PlayVideoFile(introName);
                }
#endif
            }
        }
...
void RetroEngine::Run()
{
    ...
    while (running) {
#if !RETRO_USE_ORIGINAL_CODE
        // Cap the game-update rate to refreshRate (60Hz) ALWAYS. VSync must only
        // affect presentation (tear-free), never game speed — otherwise a
        // high-refresh monitor (120/144Hz) runs the whole game 2-2.4x too fast,
        // because with vsync on the render loop, and thus the logic step, ran at
        // the monitor's rate. The software limiter now runs regardless of vsync;
        // the vsync'd present still prevents tearing, and the period stays 1/60s
        // as long as a frame's work fits in that budget.
        ...
        if (!(Engine.focusState & 1)) {
            for (int s = 0; s < gameSpeed; ++s) {
                ProcessInput();
#if RETRO_USE_NATIVE_OBJECTS
                // custom main menu: read/consume input before the stock menu runs
                if (RemasterMenu::Active())
                    RemasterMenu::Update();
                // remastered options panel: own input while the settings list is up
                else if (RemasterMenu::OptionsActive())
                    RemasterMenu::OptionsUpdate();
                // remastered in-game pause menu: own input while it's up
                else if (RemasterMenu::PauseActive())
                    RemasterMenu::PauseUpdate();
                // fast-forward the stock menu-transition timers so section
                // switches are snappy (runs regardless of which overlay is up)
                RemasterMenu::AccelMenu();
                // while the settings overlay is open, swallow this frame's game
                // input so menu navigation doesn't reach gameplay
                SettingsOverlay::FreezeGameInput();
#endif
...
bool RetroEngine::LoadGameConfig(const char *filePath)
{
    ...
    if (LoadFile(filePath, &info)) {
        ...
        // SonicCD-Native: the [Game] "Platform" setting doubles as a Mobile/PC toggle
        // for the on-screen touch controls. The TouchControls object hides itself when
        // Options.TouchControls is false, so Standard/PC hides the D-pad + buttons and
        // Mobile shows them. An explicit DisableTouchControls=true always hides them.
        {
            bool mobile = !StrComp(Engine.gamePlatform, "Standard");
            SetGlobalVariableByName("Options.TouchControls", (mobile && !disableTouchControls) ? 1 : 0);
        }
