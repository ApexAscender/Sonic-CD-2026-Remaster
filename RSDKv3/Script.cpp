...
// Full RetroScript removal: every object runs natively (verified 0 unbound across
// all scenes), so we skip loading/compiling script code entirely. Object names &
// paths still come from GameConfig/StageConfig (not RetroScript), and native
// objects run via the RUN_SUB gate in Object.cpp/Drawing.cpp. Flip to false to
// fall back to the interpreter (hybrid) for debugging.
bool nativeObjectsOnly = true;
#endif
...
void ParseScriptFile(char *scriptName, int scriptID)
{
    ...
    if (nativeObjectsOnly)
        return; // native object runs instead; no script compiled
#endif
...
#if RETRO_USE_NATIVE_OBJECTS
    if (nativeObjectsOnly)
        return; // RetroScript removed — native objects run instead
#endif
    char scriptPath[0x40];
...
{
#if RETRO_USE_NATIVE_OBJECTS
    // Native-object migration hook: if a hand-written C++ object is registered for
    // this (objectType, sub), run it and skip the interpreter entirely. Unregistered
    // types fall through unchanged, so this is a no-op until objects are ported.
    if (ProcessNativeObject(objectEntityList[objectLoop].type, scriptSub))
        return;
#endif
