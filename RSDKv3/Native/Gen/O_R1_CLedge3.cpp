...
// AUTO-GENERATED from CLedge3.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_R1_CLedge3::ObjectStartup()
{
    LoadSpriteSheet("R1/Objects2.gif");
    AddSpriteFrame(-8, -32, 16, 48, 163, 1);
    scriptEng.arrayPosition[0] = 32;
    while (scriptEng.arrayPosition[0] < 1056) {
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Crumble Ledge 3")) {
    ...
    switch (self->state) {
        ...
        if (self->values[2] == 12) {
            ...
            if (self->values[1] > self->values[0]) {
                ...
                CreateTempObject(TypeNameID("Crumble Ledge 3"), 0, scriptEng.tempValue[0], self->YPos);
...
void O_R1_CLedge3::ObjectPlayerInteraction()
{
    Entity *self = Self();

    if (self->state == 0) {
        PlayerObjectCollision(C_PLATFORM, self->values[0], -17, self->values[1], 16);
        if (CheckResult() == 1) {
            self->values[2] = 1;
            self->priority = 1;
            if (pXPos() > self->XPos) {
