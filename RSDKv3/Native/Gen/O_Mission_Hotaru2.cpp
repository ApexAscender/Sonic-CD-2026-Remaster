...
// AUTO-GENERATED from Hotaru2.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_Mission_Hotaru2::ObjectStartup()
{
    ...
    while (scriptEng.arrayPosition[0] < 1056) {
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Hotaru")) {
            objectEntityList[scriptEng.arrayPosition[0]].type = TypeNameID("Hotaru2");
        }
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Hotaru2")) {
...
void O_Mission_Hotaru2::ObjectMain()
{
    Entity *self = Self();

    if (GVar("game.playMode") == GVar("BOOT_PLAYMODE_BOSSRUSH")) {
        ...
        if (self->propertyValue == 0) {
            switch (self->state) {
                ...
                if (self->values[0] < 60) {
                    ...
                    self->values[0] = 0;
                    objectEntityList[objectLoop + 1].type = TypeNameID("Hotaru2");
                ...
                break;
                case 2:
                if (objectEntityList[objectLoop + 1].type != TypeNameID("Hotaru2")) {
                    self->state++;
                } else {
                ...
                if (self->values[0] < 60) {
                    ...
                    self->values[0] = 0;
                    objectEntityList[objectLoop + 2].type = TypeNameID("Hotaru2");
                    ...
                    objectEntityList[objectLoop + 3].type = TypeNameID("Hotaru2");
            ...
            switch (self->state) {
                ...
                scriptEng.tempValue[0] &= 1;
                if (scriptEng.tempValue[0] == 0) {
                    CreateTempObject(TypeNameID("Blank Object"), 0, self->XPos, self->YPos);
...
void O_Mission_Hotaru2::ObjectPlayerInteraction()
{
    Entity *self = Self();
...
void O_Mission_Hotaru2::ObjectDraw()
{
    Entity *self = Self();
...
void O_Mission_Hotaru2::Hotaru2_Set_Destination()
{
    Entity *self = Self();
...
const Native::NativeFunctionDef O_Mission_Hotaru2::funcs[1] = {
    { "Hotaru2_Set_Destination", O_Mission_Hotaru2::Hotaru2_Set_Destination },
};
