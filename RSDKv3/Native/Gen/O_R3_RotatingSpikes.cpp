...
#include "O_R3_RotatingSpikes.hpp"
...
void O_R3_RotatingSpikes::ObjectStartup()
{
    LoadSpriteSheet("R3/Objects.gif");
    AddSpriteFrame(-8, -8, 16, 16, 181, 143);
    AddSpriteFrame(-8, -8, 16, 16, 181, 160);
        ...
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Rotating Spikes")) {
            switch (objectEntityList[scriptEng.arrayPosition[0]].propertyValue) {
                ...
                if (objectEntityList[24].propertyValue > 1) {
                    ...
                    objectEntityList[scriptEng.arrayPosition[0]].values[2] = 3;
                }
                break;
                case 1:
                objectEntityList[scriptEng.arrayPosition[0]].values[1] = -2;
...
void O_R3_RotatingSpikes::ObjectMain()
{
    Entity *self = Self();
...
void O_R3_RotatingSpikes::ObjectPlayerInteraction()
{
    Entity *self = Self();
...
void O_R3_RotatingSpikes::ObjectDraw()
{
    Entity *self = Self();
