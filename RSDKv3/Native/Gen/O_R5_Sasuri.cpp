...
// AUTO-GENERATED from Sasuri.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_R5_Sasuri::ObjectStartup()
{
...
void O_R5_Sasuri::ObjectMain()
{
    ...
    if (self->propertyValue == 0) {
        switch (self->state) {
            case 0:
            ObjectTileCollision_Script(0, 0, 13, 0);
            if (CheckResult() == 1) {
                self->state = 1;
            } else {
                self->YPos += 0x10000;
            ...
            ObjectTileGrip_Script(0, 0, 13, 0);
            if (CheckResult() == 0) {
                self->state = 2;
                self->values[0] = 0;
                self->direction = 1;
...
void O_R5_Sasuri::ObjectPlayerInteraction()
{
    Entity *self = Self();
...
void O_R5_Sasuri::ObjectDraw()
{
    ...
    if (self->propertyValue == 0) {
        ...
        if (self->frame < 30) {
            ...
            DrawSpriteFX_Script(8, FX_FLIP, self->XPos, self->YPos);
            DrawSpriteFX_Script(1, FX_FLIP, self->XPos, self->YPos);
            DrawSpriteFX_Script(9, FX_FLIP, self->XPos, self->YPos);
        }
        switch (self->state) {
            case 1:
            case 2:
            case 3:
            case 5:
            DrawSpriteFX_Script(12, FX_FLIP, self->XPos, self->YPos);
            break;
            case 4:
            DrawSpriteFX_Script(13, FX_FLIP, self->XPos, self->YPos);
            break;
        }
    }
}
