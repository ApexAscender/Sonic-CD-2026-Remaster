...
#include "O_DAGarden_SlowBird.hpp"
...
// AUTO-GENERATED from SlowBird.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_DAGarden_SlowBird::ObjectStartup()
{
...
void O_DAGarden_SlowBird::ObjectMain()
{
    Entity *self = Self();

    self->values[3]++;
    self->values[3] %= 192;
    if (self->values[3] > 133) {
        if (self->direction == 0) {
            self->XPos += 0x10000;
        } else {
            self->XPos -= 0x10000;
...
void O_DAGarden_SlowBird::ObjectDraw()
{
    Entity *self = Self();
