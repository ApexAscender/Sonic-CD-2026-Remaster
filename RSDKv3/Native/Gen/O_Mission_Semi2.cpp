...
// AUTO-GENERATED from Semi2.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_Mission_Semi2::ObjectStartup()
{
    ...
    while (scriptEng.arrayPosition[0] < 1056) {
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Semi")) {
            objectEntityList[scriptEng.arrayPosition[0]].type = TypeNameID("Semi2");
        }
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Semi2")) {
...
void O_Mission_Semi2::ObjectMain()
{
    Entity *self = Self();
    ...
    switch (self->state) {
        ...
        if (self->propertyValue == 0) {
            if (self->values[0] == 0) {
                self->values[0] = 30;
                CreateTempObject(TypeNameID("SemiBomb2"), 0, self->XPos, self->YPos);
...
void O_Mission_Semi2::ObjectPlayerInteraction()
{
    Entity *self = Self();
...
void O_Mission_Semi2::ObjectDraw()
{
    Entity *self = Self();
