...
#include "O_R4_WaterStreamR2.hpp"
...
// AUTO-GENERATED from WaterStreamR2.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_R4_WaterStreamR2::ObjectStartup()
{
    scriptEng.arrayPosition[0] = 32;
    while (scriptEng.arrayPosition[0] < 1056) {
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Water Stream R2")) {
...
void O_R4_WaterStreamR2::ObjectPlayerInteraction()
{
    Entity *self = Self();

    if (objectEntityList[objectLoop - 1].propertyValue == 1) {
        self->propertyValue = 1;
        PlayerObjectCollision(C_TOUCH, self->values[1], -32, self->values[2], 32);
        if (CheckResult() == 1) {
