...
// AUTO-GENERATED from CLedge4.txt by retroscript2cpp.py. Faithful transliteration;
// review, then move to hand-maintained once verified.

void O_R1_CLedge4::ObjectStartup()
{
    LoadSpriteSheet("R1/Objects2.gif");
    AddSpriteFrame(-8, -40, 16, 64, 163, 1);
    scriptEng.arrayPosition[0] = 32;
    while (scriptEng.arrayPosition[0] < 1056) {
        if (objectEntityList[scriptEng.arrayPosition[0]].type == TypeNameID("Crumble Ledge 4")) {
    ...
    switch (self->state) {
        ...
        if (self->values[2] == 12) {
            ...
            if (self->values[1] > self->values[0]) {
                ...
                CreateTempObject(TypeNameID("Crumble Ledge 4"), 0, scriptEng.tempValue[0], self->YPos);
...
void O_R1_CLedge4::ObjectPlayerInteraction()
{
    Entity *self = Self();
