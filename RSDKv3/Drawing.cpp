...
#if RETRO_USE_NATIVE_OBJECTS
    // Draw the settings overlay over the finished frame (software path uses
    // frameBuffer directly; this is a no-op when the overlay is closed).
    if (renderType == RENDER_SW)
...
#if RETRO_USE_NATIVE_OBJECTS
            RemasterMenu::RenderAll(Engine.renderer);
#endif
            // finally present it
...
void DrawObjectList(int Layer)
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        if (type) {
            activePlayer = 0;
#if RETRO_USE_NATIVE_OBJECTS
            if (NativeObjectHasSub(type, SUB_DRAW) || scriptCode[objectScriptList[type].subDraw.scriptCodePtr] > 0)
#else
